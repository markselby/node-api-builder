...
p.loadRoutes = function (app) {
  var routes;
  routes = grunt.file.readYAML(process.cwd() + '/config/routes.yml');

  // console.log(routes);
  // try {
  //   // routes = require(path.join(this.root, 'config/routes'));
  // } catch (e) {
  //   throw 'Please define a routes file : /config/routes.yml';
  // }
  routes.forEach(function (r, i) {
    ...
    if (!global.controllers[target[0]]) {
      throw 'Route ' + (i + 1) + ' Controller (' + target[0] + ') doesn\'t exist : ' + Util.inspect(r);
    }
    if (!global.controllers[target[0]][target[1]]) {
      throw 'Route ' + (i + 1) + ' Function (' + target[1] + ') doesn\'t exist : ' + Util.inspect(r);
    }
