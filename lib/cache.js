...
function storeInRedis(req, res, cb) {
  var lifetime = res.cache;

  if(isNaN(lifetime) || res.statusCode !== 200) {
    // console.log('Not caching : ' + req.originalUrl);
    cb(res.body);
    return;
  }
...
function cache(req, res, next) {
  ...
  res.end = function(chunk, encoding) {
    ...
    storeInRedis(req, res, function(body) {
      res.end = end;              // Restore the original end function to its rightful place
      res.end(body, encoding);   // Respond to the user with the compressed result
    });
  }
  next();
};
