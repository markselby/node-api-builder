...
function storeInPostgres(cacheKey, hash, body) {
  ...
  // console.log('Body is buffer : ' + (Buffer.isBuffer(body) ? true : false));
  // console.log(i(body));

  // db.named('page-cache-update', [
  //   config.server.name, cacheKey, hash.etag, hash.max_age, hash.content_type, hash.content_length, hash.content_encoding,
  //   // '\\x' + body.toString('hex')
  //   // body.toString('utf8')
  //   body
  // ])
...
function storeInCache(req, res, body, cb) {
  ...
  if(!config.server.cache || isNaN(hash.max_age) || (res.statusCode !== 200 && res.statusCode !== 304)) {
    cb(hash, body);
    return;
  }
